...
source('eFormsOrganizeData_byTable.r')
...
setwd(paste(getwd(),"/NRS18_AL_10001_2",sep=''))
# Create list of files in working directory with extension of json or JSON
filelist <- list.files(pattern='json|JSON')
# Specifically exclude files with TRACKING in the name because this can vary over time
...
for(i in 1:length(filelist)){
  ...
  subName.out <- str_extract(fileName,"[:alnum:]+\\_[:alpha:]+\\_[:alnum:]+\\_[:alnum:]\\_")
