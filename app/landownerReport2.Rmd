...
## Assessment info
assessment <- dplyr::filter(z[['ASSESSMENT']], PARAMETER %in% c('WEATHER','CONDITIONS','OBSERVATIONS','BEAVER_FLOW_MOD')) %>% 
  dplyr::select(PARAMETER, RESULT) %>%
...
samplesTaken <- unique(z[['SAMPLES']]$SAMPLE_TYPE)[!(unique(z[['SAMPLES']]$SAMPLE_TYPE) %in% 'SAMP')]
...
## Fish 
fishTaxa <- dplyr::filter(z[['FISH']], SAMPLE_TYPE == 'FISH') %>%
